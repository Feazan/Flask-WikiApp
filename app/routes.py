...
from app import app
from app.forms import SubmissionForm, LoginForm, RegistrationForm, EditWiki
...
def edit(postid):
    if current_user.is_authenticated:
        ...
        if form.validate_on_submit():
            ...
            edited_file = open(filename, "w")
            edited_file.write("\n".join([current_posts[0].title, form.text.data]))
            edited_file.close()
            # Edit the existing entry
            new_text = form.text.data
            # print(new_text)
            current_posts[0].body = new_text
            db.session.commit()
            flash('Topic {} has been edited.'.format(
                current_posts[0].title))
            return redirect(url_for('index'))
        form.text.data = current_posts[0].body
        return render_template('edit.html', title='Edit Entry', form=form, post=current_posts[0])
    return render_template('notloggedin.html')
